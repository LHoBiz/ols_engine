...
APDATA = lambda : open(os.path.join(os.path.dirname(__file__), 'AirportData.csv'),'r')
...
class Data:
    def __init__(self, name=None):
        ...
        if name is None:
            ...
            KML_NAME = name# input('Type the name of the airport: ')
        ...
        You may either:
            ...
            ChooseRwy = str(1)# runway.name# input('Enter number: ')
        def SelectedData(rwyDat,ChooseRwy,KML_NAME):
            print(ChooseRwy , rwyDat , KML_NAME , rwyDat)
            for x in range(len(rwyDat)):
                
                if ChooseRwy == rwyDat[x][0] and KML_NAME == rwyDat[x][1]:
                    return rwyDat[x]
        self.KML_NAME = KML_NAME
        ap = self.ap = SelectedData(rwyDat,ChooseRwy,KML_NAME)[1]        
        Zone = self.Zone = SelectedData(rwyDat,ChooseRwy,KML_NAME)[4]
        NTSH = self.NTSH = SelectedData(rwyDat,ChooseRwy,KML_NAME)[5]
        NTE = self.NTE = SelectedData(rwyDat,ChooseRwy,KML_NAME)[6]
        NTN = self.NTN = SelectedData(rwyDat,ChooseRwy,KML_NAME)[7]
        STSH = self.STSH = SelectedData(rwyDat,ChooseRwy,KML_NAME)[12]
        STE = self.STE = SelectedData(rwyDat,ChooseRwy,KML_NAME)[13]
        STN = self.STN = SelectedData(rwyDat,ChooseRwy,KML_NAME)[14]
        ...
        KML_PATH = self.KML_PATH = os.path.join(os.getcwd(),'v0.2')
        if not os.path.exists(KML_PATH):
            os.mkdir(KML_PATH)

        self.completeName = completeName = os.path.join(KML_PATH, KML_NAME+".kml")
        self.NewKML = NewKML = KML_NAME
        self.f = f = open(completeName, 'w')
        
        self.ReportData = ReportData = os.path.join(KML_PATH,"Obstacle Report"+".txt")
        self.rep=rep=open(ReportData, 'w')
        ...
        self.projType = projType = isProj(projType)
        ...
        if projType == 'WGS84':
            ...
            while isLAT_DD(NTlat,ns) == False:
                ...
                NTlat = raw_input("Enter Northern Threshold latitude in decimal degrees: ")       
            self.NTlat = NTlat=isLAT_DD(NTlat,ns)
            ...
            self.NTlon = NTlon=isLON_DD(NTlon,ew)
            ...
            while isLAT_DD(STlat,ns) == False:
                ...
                #print 'Please ensure you enter a valid response only. Try again.'
                STlat = raw_input("Enter Northern Threshold latitude in decimal degrees: ")       
            self.STlat = STlat=isLAT_DD(STlat,ns)
            ...
            while isLON_DD(STlon,ew) == False:
                ...
                #print 'Please ensure you enter a valid response only. Try again.'
                NTlon = raw_input("Enter Northern Threshold latitude in decimal degrees: ")       
            self.STlon = STlon =isLON_DD(STlon,ew)
        ...
        while CodeN(CN) == False:
            ...
            self.CN = CN = raw_input("Enter Runway Code Number: ")
        if CN == 'ALA':
            self.CN = CN = 'ALA'
        else:
            self.CN = CN = float(CN)
        ...
        while CodeL(CL) == False:   
            ...
            self.CL = CL = raw_input("Enter Runway Code Letter: ")
        if CL == 'ALA':
            self.CL = CL = None
        else:
            self.CL = CL = CodeL(CL)
        ...
        if NIns == 'Y':
            ...
            while IsY1Y2Y3N(NPrc,CN) == False:   
                ...
                NPrc = raw_input("Is the Southern Approach runway a precision instrument runway (Y1 / Y2 / Y3 / N)? ")
            self.NPrc = NPrc = IsY1Y2Y3N(NPrc,CN)
            if NPrc == 'Y1' or NPrc == 'Y2' or NPrc == 'Y3':
                if CN == 1 or CN == 2 or CN == 3 or CN == 4:
                    ...
                    while IsNum(NBLDist) == False:
                        ...
                        NBLDist = raw_input("For the Northern Approach Baulked Landing surface, enter the distance of the surface from the threshold: ")
                    self.NBLDist = float(NBLDist)
            if NPrc == 'N':
                self.NRunwayInfo = ' Instrument non-precision'
            elif NPrc == 'Y1':
                self.NRunwayInfo = ' Instrument precision (cat1)'
            elif NPrc == 'Y2':
                self.NRunwayInfo = ' Instrument precision (cat2)'
            elif NPrc == 'Y3':
                self.NRunwayInfo = ' Instrument precision (cat3)'
        elif NIns == 'N':
            self.NRunwayInfo = ' Non-instrument'
        ...
        if SIns == 'Y':
            ...
            while IsY1Y2Y3N(SPrc,CN) == False:   
                ...
                #print 'If you need to use another code number, press Ctrl+C to start again.'
                SPrc = raw_input("Is the Northern Approach runway a precision instrument runway (Y1 / Y2 / Y3 / N)? ")
            self.SPrc = SPrc = IsY1Y2Y3N(SPrc,CN)
            if SPrc == 'Y1' or SPrc == 'Y2' or SPrc == 'Y3':
                if CN == 1 or CN == 2 or CN == 3 or CN == 4:
                    ...
                    while IsNum(SBLDist) == False:
                        ...
                        SBLDist = raw_input("For the Northern Approach Baulked Landing surface, enter the distance of the surface from the threshold: ")
                    self.SBLDist = float(SBLDist)
            if SPrc == 'N':
                self.SRunwayInfo = ' Instrument non-precision'
            elif SPrc == 'Y1':
                self.SRunwayInfo = ' Instrument precision (cat1)'
            elif SPrc == 'Y2':
                self.SRunwayInfo = ' Instrument precision (cat2)'
            elif SPrc == 'Y3':
                self.SRunwayInfo = ' Instrument precision (cat3)'
        elif SIns == 'N':
            self.SRunwayInfo = ' Non-instrument'
    ...
    ##    SCLWY = float(SCLWY)
        ...
        while IsYyNn(NMTOW22700kg) == False:   
            ...
            #print 'Please ensure you enter either Y or N only. Try again.'
            NMTOW22700kg = raw_input("Do aircraft with MTOW >= 22700 kg operate on the northern take-off runway? (Y / N): ")
        self.NMTOW22700kg = IsYyNn(NMTOW22700kg)
        ...
        if NMTOW22700kg == 'Y':
            ...
            while IsYyNn(NMTOW5700kg) == False:   
                ...
                #print 'Please ensure you enter either Y or N only. Try again.'
                NMTOW5700kg = raw_input("Do aircraft with MTOW >= 5700 kg operate on the northern take-off runway? (Y / N): ")
            self.NMTOW5700kg = IsYyNn(NMTOW5700kg)
        ...
        self.SMTOW22700kg = IsYyNn(SMTOW22700kg)

        if SMTOW22700kg == 'Y':
            self.SMTOW5700kg = 'Y'
        elif SMTOW22700kg == 'N':
            #SMTOW5700kg = raw_input("Do aircraft with MTOW >= 5700 kg operate on the southern take-off runway? (Y / N): ")
            self.SMTOW5700kg = SMTOW5700kg= 'Y'
            while IsYyNn(SMTOW5700kg) == False:   
                #print ''
                #print 'Please ensure you enter either Y or N only. Try again.'
                SMTOW5700kg = raw_input("Do aircraft with MTOW >= 5700 kg operate on the southern take-off runway? (Y / N): ")
            self.SMTOW5700kg = IsYyNn(SMTOW5700kg)
        self.MTOW5700kg = IsYyNn('Y')
        ...
        while IsYyNn(JetTransport) == False:   
            ...
            #print 'Please ensure you enter either Y or N only. Try again.'
            JetTransport = raw_input("Are there Jet Transport Aeroplanes operating on the runway?(Y / N): ")
        self.JetTransport = IsYyNn(JetTransport)
        ...
        while IsYyNn(NTOTurn15d) == False:   
            ...
            #print 'Please ensure you enter either Y or N only. Try again.'
            NTOTurn15d = raw_input("Is there a northern take-off procedure that has a turn >15 degrees? (Y / N): ")
        self.NTOTurn15d =   IsYyNn(NTOTurn15d)
        ...
        while IsYyNn(STOTurn15d) == False:   
            ...
            #print 'Please ensure you enter either Y or N only. Try again.'
            STOTurn15d = raw_input("Is there a southern take-off procedure that has a turn >15 degrees? (Y / N): ")
        self.STOTurn15d =   IsYyNn(STOTurn15d)
